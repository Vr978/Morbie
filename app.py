...
def predict():
      if request.method == 'POST':
        ...
        model_response = gmodel.generate_content( f"""
                    You are a healthcare expert. Based on the user's input, provide resources for managing the predicted disease. Here is the format for your response:

User Input: {predicted_disease}

Resources for {predicted_disease}:

Give details about {predicted_disease} and give statistics related to its severness and for how long one has wait before it gets harmful. Also state it's mostly in masculine or feminine gender.
...
Also give some details about treatment processes doctors are using to treat this {predicted_disease}

IMPORTANT: Ensure that all links are functional and provide valuable information. Use emojis to enhance engagement!
Always tell user to contact with real doctor before applying any of above.

Note: Include emojis related to healthcare in answer.

                    """)
        response = model_response.text 

        # Return the result as JSON
        return {'disease': predicted_disease,'response': response}
